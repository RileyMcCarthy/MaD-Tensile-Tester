...
def writeSerial(serial, data, n=-1):
    if (n == -1):
...
def setFlashAddr(serial, addr):
    writeCmd(serial, CMD_FLASHDATA)
    writeSerial(serial, convert_to_bytes(ctypes.c_int32(addr)),
...
def setMotionMode(serial, mode):
    print("mode: "+str(convert_to_bytes(ctypes.c_int(mode))) +
          " " + str(ctypes.sizeof(ctypes.c_int)))
    writeCmd(serial, CMD_MOTIONMODE)
    writeSerial(serial, convert_to_bytes(
