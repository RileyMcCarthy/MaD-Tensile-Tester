...
def read_spi(gpio, level, tick):
    ...
    if num_bytes == ctypes.sizeof(MonitorData):
        ...
        data = json.dumps({"time": monitorData.timems/1000.0, "position": monitorData.position,
                          "force": monitorData.force, "setpoint": monitorData.setpoint})
        print(data)
    else:
        print("Error reading SPI, bytes to not match. Got " +
              str(num_bytes)+" bytes, expected "+str(ctypes.sizeof(MonitorData)))
