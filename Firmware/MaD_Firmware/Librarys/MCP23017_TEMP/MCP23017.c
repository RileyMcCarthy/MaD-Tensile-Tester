...
MCP23017 *mcp23017_create(uint8_t address, int sda, int scl)
{
	...
	i2c_slow_open(&(mcp23017->bus), scl, sda, 1, 100);
...
Error mcp23017_init(MCP23017 *mcp23017)
{
	...
	if (!write_register(mcp23017, IOCON, 0b00000000))
	{
		return MCP23017_NOT_FOUND;
	}
	write_register(mcp23017, 0, 255);
	while (1)
	{
		printf("HIGH\n");
		write_register(mcp23017, 0x12, 255);
		_waitms(500);
		write_register(mcp23017, 0x12, 0);
		printf("LOW\n");
		_waitms(500);
	}
...
void mcp23017_pin_mode(MCP23017 *mcp23017, uint8_t pin, uint8_t mode, bool inverted)
{
	...
	if (mode == MCP23017_INPUT || mode == MCP23017_INPUT_PULLUP)
		bitSet(iodir, pin);
	else
		bitClear(iodir, pin);
